...
def fixedhough():
  ...
  cv2.line(image,(0,200),(640,200),(0,255,0),2)
...
#     for x1,y1,x2,y2 in lines[x]:
        ...
        ##compute difference with fixed horizontal line
        p3 = np.array([x1,y1])
        p4 = np.array([x2,y2])
        derp = seg_intersect(p1,p2,p3,p4)
        print(derp)
    
  cv2.imshow('derp',image)
  cv2.waitKey(20000) & 0xFF == ord('q')
...
def findClosestRightLine(lines, middle):
  for x in range(0, len(lines)):
    for x1,y1,x2,y2 in lines[x]:
...
def findClosestLeftLine(lines, middle):
...
def hough():
  ...
  slope_thresh = 0.1
  distance_thresh = 50
  camera = cv2.VideoCapture("video_new.h264.MKV")
  while True:
  ...
  #image = frame.array
    ...
    if lines is not None:
      ...
      for x in range(0, len(lines)):
        for x1,y1,x2,y2 in lines[x]:
          a=findClosestRightLine(lines, 640/2)
          b=findClosestLeftLine(lines, 640/2)
          #print (lines[x])
          #print ("-------")
          
          cv2.line(image,(a[0],a[1]),(a[2],a[3]),(0,255,0),5)
          cv2.line(image,(b[0],b[1]),(b[2],b[3]),(255,0,0),5)
          cv2.line(image,(x1,y1),(x2,y2),(0,0,255),2)
  ...
  #      if x in ignorelist:
  #        continue
  #      for rho,theta in lines[x]:    #only 1 element with 2 variables
  #        for y in range (x,len(lines)):
  #          for rho_2,theta_2 in lines[y]:
  #            if rho + rho_thresh > rho_2 and rho - rho_thresh < rho_2 and theta + theta_thresh > theta_2 and theta - theta_thresh < theta_2:
  #              #lines = np.delete(lines,y)
  #              ignorelist.append(y)
  #              i = i+1
  #              #print('lineduplicate')
  #        a = np.cos(theta)
  ...
  #        
  #        if i>0:
  #          cv2.line(image,(x1,y1),(x2,y2),(0,0,255),i)
...
def colordetection_video():
  camera = cv2.VideoCapture("video3.h264.640.MKV")
  while True:
  # grab the current frame
    (grabbed, image) = camera.read()
...
YellowLower = (13, 130, 130)
YellowUpper = (20, 255, 255)

TilesLower = (0,0,0)
TilesUpper = (255,255,175)

left_nor = cv2.imread("left_nor.png",0)
stop_nor = cv2.imread("stop_nor.png",0)
uturn_nor = cv2.imread("uturn_nor.png",0)
right_nor = cv2.imread("right_nor.png",0)
straight_nor = cv2.imread("straight_nor.png",0)
circle_and = cv2.imread("circle_and.png",0)
